...
class Delete():
    ...
    def __delete_lib_related(self, lib_id):
        ...
        obj = self.obj_instance.get('section').query.filter(
            text('libid==%s' % lib_id)).all()
        for a_obj in obj:
            self.__delete_seciton_related(a_obj.id)
            self.obj_instance.get('section').query.filter(
                text('id==%s' % a_obj.id)).delete(synchronize_session=False)
            db_session.commit()
    ...
    def __delete_seciton_related(self, section_id):
        ...
        级联删除章节关联数据
        '''
        obj = self.obj_instance.get('subject').query.filter(
            text('sectionid==%s' % section_id)).all()
        for a_obj in obj:
            self.__delete_subject_option_and_other(a_obj.id)
            self.obj_instance.get('subject').query.filter(
                text('id==%s' % a_obj.id)).delete(synchronize_session=False)
            db_session.commit()
